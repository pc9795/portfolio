...
public class BlogPostResource {
    private static final Map<String, Integer> THREE_LETTER_MONTH_NAMES_TO_MONTH_NO = new HashMap<>() {{
        put("jan", 1);
        put("feb", 2);
        put("mar", 3);
        put("apr", 4);
    ...
    public Page<BlogPostResponse> getByYear(@PathVariable("year") String yearStr, Pageable pageable) {
        ...
        return blogPostCache.findByYearCreatedAtBetween(firstMinuteOfYear, lastMinuteOfYear, pageable).map(blogPostMapper::toBlogPostResponse);
    }

    @GetMapping("/year/{year}/month/{month}")
    public Page<BlogPostResponse> getByMonthYear(@PathVariable("year") String yearStr,
                                                 @PathVariable("month") String monthStr, Pageable pageable) {
        Optional<Integer> maybeYear = getYear(yearStr);
        if (!maybeYear.isPresent()) {
            throw new BadDataException(String.format("%s is not a valid year", yearStr));
        }
        ...
        return blogPostCache.findByYearCreatedAtBetween(firstMinuteOfMonth, lastMinuteOfMonth, pageable).map(blogPostMapper::toBlogPostResponse);
    }

    @GetMapping("/search/{searchText}")
    public Page<BlogPostResponse> search(@PathVariable("searchText") String searchText, Pageable pageable) {
        return blogPostCache.findByHeadingContaining(searchText, pageable).map(blogPostMapper::toBlogPostResponse);
    }
    ...
    public BlogPostWithContentResponse getByName(@PathVariable("blogPostName") String blogPostName) {
        ...
        if (blogPost == null) {
            throw new ResourceNotFoundException(String.format("Blog post with name: %s is not found", blogPostName));
        }
