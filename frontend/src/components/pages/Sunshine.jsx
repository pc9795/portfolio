    ...
    return <div style={styles.mainContainer} className="container">
        ...
        <div className="row mt-5 b">
            ...
            <div className="col-12 col-sm-6 b text-center mt-3">
                {getStartBtn(poemShown, setPoemShown, setShowBtnShown)}
                {getPoem(poemShown, secretShown)}
                {getShowBtn(showBtnShown, showBtnClickCount, setShowBtnClickCount, setShowBtnShown, setSecretShown)}
            </div>
            <div className="col-12 col-sm-3 b mt-3">
...
function getStartBtn(poemShown, setPoemShown, setShowBtnShown) {
    ...
    return <button style={styles.startBtn} onClick={() => {
        handleStartBtnClick(setPoemShown, setShowBtnShown)
...
function getShowBtn(showBtnShown, showBtnClickCount, setShowBtnClickCount, setShowBtnShown, setSecretShown) {
    ...
    return <button style={styles.showBtn}
                   onClick={() => handleShowBtnClick(showBtnClickCount, setShowBtnClickCount, setShowBtnShown, setSecretShown)}
                   className="btn btn-info b">
        <span className="display-linebreak">{getShowBtnText(showBtnClickCount)}</span>
    </button>;
}
